...
- ビデオ出力には下記リソースを利用します.  
  ...
  - タイマー Timer2  
  - SPI1 or SPI2 (begin()の第2引数spinoで指定可能、SPI1がデフォルト)  
  - DMA1 CH3  
...
接続端子  
PA1: 同期信号  
PA7: 映像信号(SPIにSPI1を利用した場合)  
※SPIにSPI2を利用する場合、映像信号出力ピンはPB15となります.  
...
- 引数  
  cnt :垂直同期信号補正値（指定した水平走査線数分、タイミングを調整する）   
  hcnt:横画像表示開始位置補正（指定したクロック数分、開始位置を補正する）  
  vcnt:縦画像表示開始位置補正（指定した走査線数分、開始位置を補正する）  
- 戻り値  
  なし  
- 説明  
  NTSCビデオ出力に関するタイミング等の補正を行います。  
  利用しているモニターディスプレイの映像がスクロールしたり、映像が欠けたりする場合、この関数により補正出来る場合があります。  
  垂直同期が取れない（スクロールする、映像の下部がにじむ）場合は、cntを調整、画像位置を補正する場合は、hcnt、vcntにて補正が出来ます。  
  この関数は、begin()関数の前に呼び出して下さい。  
...
- 引数  
   mode :画面モード  
    spino:利用するSPIの指定 1:SPI2(デフォルト)、2:SPI2  
   extram:外部獲得フレームバッファ用メモリアドレス、NUULの場合は指定なし  
